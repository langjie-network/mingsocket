...
class UdpSocketClientList extends BaseTableList  {
        ...
        return(
            <p>
                <a href="javascript:void(0)" onClick={async ()=>{
                    if(row.connected){
                        let r=  await MIO.tcpSocketDisConnect({
                            remoteIp: Common.dec2ip(row.hotMasterIp),
                            remotePort:row.msgPort,
                        });
                    }else {
                        let r=  await MIO.tcpSocketConnect({
                            remoteIp: Common.dec2ip(row.hotMasterIp),
                            remotePort:row.msgPort,
                        });
                    }
                }} style={{marginLeft: 10}}>{row.connected ? "断开连接":"连接"}</a>
                &nbsp;  &nbsp;
    ...
    componentDidMount() {
        M.Component.UdpSocketClientList=this;
        this.fetch();
    };
    ...
    async fetch() {
        this.setState({ loading: true,callVisable:false  });
            ...
            let r=await window.MIO.updSocketClientList({
                page: current,
                num: pageSize,
                keywords: keywords,
                startDate:startDate,
