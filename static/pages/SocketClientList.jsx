...
app.get("/socketClientList",async (req,res)=>{
    const queryCode=req.params;
    let apiPath=  M.config.baseUrl("/api/socketClientList");
...
class SocketClientList extends BaseTableList  {
    ...
    componentDidMount() {
        M.Component.SocketClientList=this;
        this.fetch();
    };
    ...
    async fetch() {
        this.setState({ loading: true,callVisable:false  });
            ...
            let r=await window.MIO.socketClientList({
                page: current,
                num: pageSize,
                keywords: keywords,
                startDate:startDate,
...

                {
                    this.state.callVisable?
                        <Drawer
                            ...
                            width={600}
                        >
                            <SocketClientCallForm parent={this}/>
                        </Drawer>:""
                }
