...
class SocketIoWebPlugin {
    ...
    async connect({connectGroup}){
        let that=this;
        let r= await M.request.post(M.config.plcmcontrollerHost("/open/plcm/socketio/applySocketio"),{
            "terminalType":"wx",
            "direction":2,
            "connectGroup": "img2data",
            "app":"img2data",
            "share":1
        });
        ...
        this.unionid="anonymous";
        var socket = window.io(
            that.host+ '/?clientId='+this.clientId+"&unionid="+"anonymous"
            ,{path:``});
        this.socket=socket;
